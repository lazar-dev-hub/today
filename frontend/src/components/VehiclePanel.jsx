  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-5 rounded-lg shadow border border-gray-200 space-y-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <input name="plate_number" placeholder="Plate Number" value={form.plate_number} onChange={handleChange} required className="border rounded px-3 py-2 text-sm" />
        <input name="brand" placeholder="Brand" value={form.brand} onChange={handleChange} required className="border rounded px-3 py-2 text-sm" />
        <input name="model" placeholder="Model" value={form.model} onChange={handleChange} required className="border rounded px-3 py-2 text-sm" />
        <input name="year_manufactured" placeholder="Year" type="number" value={form.year_manufactured} onChange={handleChange} required className="border rounded px-3 py-2 text-sm" />
        <select name="vehicle_type" value={form.vehicle_type} onChange={handleChange} className="border rounded px-3 py-2 text-sm">
        ...
        <input name="purchase_price" placeholder="Purchase Price" type="number" value={form.purchase_price} onChange={handleChange} required className="border rounded px-3 py-2 text-sm" />
              ...
              vehicles.map((v) => (
                  <tr key={v.vehicle_id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2">
                      <button onClick={() => handleDelete(v.vehicle_id)} className="text-red-500 hover:underline text-xs font-semibold">Delete</button>
                    </td>
                  </tr>
                ))
              )}
